...
- 某些 provider 端点不接受 Kivio 发的 `prompt_cache_key`/`promptCacheKey` 会话缓存键字段（实测 Google Gemini OpenAI-compat 端点返回 `400 Unknown name "promptCacheKey"`）。probe 会把该错误如实写进 `result.json.error`。**这是独立于 probe 的真实 provider 兼容 bug**（`ca7be76` 会话亲和引入），该字段应按 provider 门控——待单独任务修复。
