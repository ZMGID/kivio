...
export default function Lens() {
  ...
  const flyBarToAnchor = async (
    ...
    if (!keepFullscreen) {
      // 浮动模式：fly 期间窗口保持全屏，bar 在 webview 内用 CSS transition 平滑飞到锚点；
      // fly 完成后把窗口一次性 snap 到浮动位置/尺寸。
      //
      // 旧实现：JS RAF 每帧调 lensSetFloating（→ Win32 SetWindowPos），380ms 内做 ~23 次。
      // SetWindowPos 是重操作，会触发 DWM 合成 + WebView2 内部 resize 协调；多次 hide/show
      // 周期后 DWM/WebView2 的内部状态退化，per-frame resize 累积成可见的"乱跳"。
      // 现在 23 次合并成 1 次（fly 末尾 snap），把累积风险点完全摘掉。
      //
      // 视觉一致性：snap 时 bar 在 webview 的位置由 (anchorX, anchorY) 改成 (PADDING, PADDING)，
      // 同时窗口 origin 移到 (winOrigin_full + anchorX - PADDING, ...)。screen pos 不变。
      // 但 React 提交 barRect 与 OS 完成 resize 不能完全原子，中间 1 帧 bar 会出现在 fullscreen
      // overlay 的 (PADDING, PADDING)（≈ 整屏左上角）。用 opacity:0 把那一帧盖掉，肉眼无感。
      ...
      flushSync(() => {
        ...
        if (isTranslateMode) {
          // translate 模式截图前没渲染卡片 → 没"起点位置"，强行 fly 会从 (selectX,selectY) 开始，
          // 视觉上像是凭空从某个无关位置滑过来。保留旧行为：mount 在隐身态，等 snap 完成后 fade in。
          setBarRect({ x: FLOATING_PADDING, y: FLOATING_PADDING, width: READY_W })
          setBarIntro(false)
          setBarNoTransition(true)
        } else {
          // chat 模式：bar 已经在 select 位置，CSS transition 自然飞到 (finalX, finalY) in webview 坐标。
          setBarRect({ x: finalX, y: finalY, width: READY_W })
        }
      })
      if (floatingRebaseTimerRef.current) clearTimeout(floatingRebaseTimerRef.current)
      floatingRebaseTimerRef.current = setTimeout(() => {
        if (stageRef.current === 'select') return
        // 第 1 步：bar 隐身 + 禁 transition + 切到浮动 webview 坐标。
        // 这一帧 bar 视觉跳到 fullscreen overlay 的 (PADDING,PADDING)（屏幕左上角），
        // 但 opacity:0 看不到。
        flushSync(() => {
          setBarIntro(false)
          setBarNoTransition(true)
        ...
        // 第 2 步：等窗口完成 resize（一次 IPC，~3-5ms）。
        void (async () => {
          try {
            await api.lensSetFloating({ x: floatX, y: floatY, width: floatW, height: floatH })
          } catch (err) {
            console.error('[lens] lensSetFloating snap failed:', err)
          }
          // IPC 期间 / 后用户可能 Esc 关掉 lens（甚至已经重开了新会话），stage 切回 'select'。
          // 此时不应再 setBarIntro / setBarNoTransition，否则会污染新会话的 select 态。
          if (stageRef.current === 'select') return
          // 第 3 步：浮动尺寸生效后，bar 在 (PADDING,PADDING) of 浮动 webview = 原 anchor 屏幕位置。
          // - chat 模式：bar 之前就可见（fly 中）, 这里立即显示，避免再走一次 380ms fade-in 延迟。
          //   raf 1 复显 + 仍禁 transition → opacity 0→1 不走过渡；raf 2 再解禁。
          // - translate 模式：bar 在 fly 期间是隐身的，这里走完整的 transition fade-in，体验跟旧版一致。
          if (isTranslateMode) {
            requestAnimationFrame(() => {
              requestAnimationFrame(() => {
                setBarNoTransition(false)
