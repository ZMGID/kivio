...
export default function Lens() {
  ...
  const flyBarToAnchor = async (
    ...
    if (!keepFullscreen) {
      // 浮动模式：bar 的 CSS transition 从 select 位置飞到 (PADDING, PADDING)（webview 内坐标），
      // JS RAF 同步缩窗（lensSetFloating 每帧调一次）。两条动画用同一条 cubic-bezier，
      // 屏幕上 bar 的视觉位置 = window_origin + bar.left 是平滑的。
      //
      // 一开始改成"fly 期间窗口保持全屏 / fly 末尾一次性 snap"想绕开累积型 jitter，
      // 但 snap 时 React 提交 barRect 和 OS 完成 resize 无法完全原子 → 重新引入了
      // commit b386f0a 当年专门修过的"到位后闪一下消失再恢复"的老毛病，且整段动画
      // 也没原版顺。回到原版逐帧方案。
      //
      // 累积型 jitter 改在 Rust 侧解决：lens_set_floating 在 Windows 上改成单次原子
      // SetWindowPos（合并 set_position + set_size），把每帧的 DWM/WebView2 resize
      // 协调工作砍一半，多次会话后的累积压力大幅降低。
