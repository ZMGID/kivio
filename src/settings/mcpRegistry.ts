...
export const MCP_REGISTRY_SOURCE_OPTIONS: Array<{ value: McpRegistrySource; label: string }> = [
  { value: 'official', label: 'Official' },
  { value: 'smithery', label: 'Smithery' },
  { value: 'glama', label: 'Glama' },
]

/** 构造一个字段齐全的 ChatMcpServer 基底（enabled=false，调用方按需覆盖）。 */
function baseServer(id: string, name: string): ChatMcpServer {
  return {
    id,
    name,
    enabled: false,
...
function commandPreview(server: ChatMcpServer) {
...
function recordFromInputs(values: unknown[], target: 'env' | 'header', requiredConfig: McpRegistryConfigInput[]) {
...
function pushTemplateRequirements(
  ...
  for (const name of unresolvedTemplateNames(value)) {
    requiredConfig.push({ name, label: name, required: true, secret: false, target })
  }
}
...
function makeDraft(
  ...
  return {
    server: { ...server, enabled: status === 'ready' },
    status,
...
function pickOfficialDraft(server: RawRecord): McpRegistryInstallDraft | undefined {
  const baseName = asString(server.name) ?? 'mcp-server'
  for (const remote of asArray(server.remotes)) {
    const remoteRecord = asRecord(remote)
    const draft = buildRemoteDraft(baseName, remoteRecord, asRecord(remoteRecord.variables))
    if (draft?.status === 'ready') return draft
  }

  const drafts = asArray(server.packages)
    .map((pkg) => buildOfficialPackageDraft(baseName, asRecord(pkg)))
...
function envRecordFromJsonSchema(schema: unknown) {
  ...
  for (const [name, raw] of Object.entries(properties)) {
    const property = asRecord(raw)
    const exampleValue = objectExamples.map((example) => asConfigString(example[name])).find(Boolean)
...
function normalizeSmitheryDetail(card: McpRegistryCard, raw: unknown): McpRegistryCard {
  ...
  const stdioConnection = connections.map(asRecord).find((connection) => asString(connection.type) === 'stdio')
  ...
  const httpDraft = deploymentUrl
    ? makeDraft(
        {
          ...baseServer(slugifyServerId(card.name), card.displayName),
  ...
  return {
    ...
    installDraft,
    installUnavailableReason: installDraft ? undefined : 'manual',
    transportHints: installDraft
      ? [installDraft.server.transport === 'stdio' ? 'stdio' : 'http']
      : card.transportHints,
...
/** Smithery 搜索卡片不带连接信息，安装前按需拉详情补出 installDraft。 */
export async function resolveMcpRegistryInstallDraft(
  ...
  const detail = await fetchJson(`${SMITHERY_API_BASE}/servers/${encodeSmitheryPath(card.sourceId)}`, {
...
function replaceArgumentConfigValue(args: string[], input: McpRegistryConfigInput, value: string) {
  ...
  const placeholderIndex = args.findIndex(
    (arg) => arg === '...' || arg.includes('...') || names.some((name) => arg.includes(`{${name}}`)),
  )
...
function replaceConfigArgValue(args: string[], input: McpRegistryConfigInput, value: string) {
  const targetName = input.targetName ?? input.name
  return upsertConfigArg(args, { ...readConfigArg(args), [targetName]: value })
}

/** 把用户在 needs_config 表单里填的值回填到 server 的对应位置，得到 ready 草稿。 */
export function applyMcpRegistryInstallConfig(
  ...
  for (const input of draft.requiredConfig) {
    ...
    if (input.target === 'env') {
      server = { ...server, env: { ...server.env, [targetName]: value } }
      continue
    }
    if (input.target === 'header') {
      server = { ...server, headers: { ...server.headers, [targetName]: value } }
      continue
    }
    if (input.target === 'argument') {
      server = { ...server, args: replaceArgumentConfigValue(server.args ?? [], input, value) }
      continue
    }
    if (input.target === 'config') {
      server = { ...server, args: replaceConfigArgValue(server.args ?? [], input, value) }
      continue
    }
    server = { ...server, url: replaceUrlConfigValue(server.url ?? '', input, value) }
  }
