            ...
            {activeTab === 'general' && (
              <>
                <SettingsGroup title={lang.startsWith('zh') ? '外觀' : 'Appearance'}>
                  ...
                  <SettingRow label={t.themeColor} description={lang.startsWith('zh') ? '選擇淺色界面的背景色調。' : 'Choose the surface tint for light appearance.'}>
            ...
            {activeTab === 'webSearch' && (
              <>
                ...
                <p className="kv-row-desc px-1 py-2">
                  {lang.startsWith('zh')
                    ? 'Chat 的 web_search / web_fetch 開關在「MCP」→「Kivio 內置工具」中配置。'
                  ...
                  <SettingsGroup title={lang.startsWith('zh') ? '供應商' : 'Provider'} className="!pt-0 kv-provider-section">
                    {selectedProvider ? (() => {
                      ...
                      <p className="kv-provider-empty-hint">
                        {lang.startsWith('zh') ? '在左側選擇供應商，或使用快速預設添加。' : 'Select a provider on the left, or add one from quick presets.'}
                      </p>
                    )}
                  </SettingsGroup>
