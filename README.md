...
- 📸 **Screenshot translate** — capture any window or region, or translate selected text directly without taking a screenshot.
...
Capture any window or region — the translated card flies to where you selected, with the original text right below for reference. Prefer text selection? Highlight text anywhere and press the selected-text hotkey to open the same translation card directly.
...
- **v2.5.5** — Screenshot translation now uses true system OCR on both platforms: Apple Vision on macOS and `Windows.Media.Ocr` on Windows, then sends plain text to the selected translator. macOS OCR is split into its own lightweight Vision helper, so it no longer depends on Apple Intelligence being enabled. Release builds now fail if the required OCR helper cannot be built.
- **v2.5.4** — Screenshot translation now has a selected-text entry: highlight text and press `⌘⇧T` / `Ctrl+Shift+T` to translate it directly, reusing the Lens selection capture path without taking a screenshot. Settings adds a separate hotkey recorder for this entry and keeps duplicate-hotkey validation across all shortcuts. The selected-text result card opens as a draggable floating window, respects streaming/direct/original/thinking settings, and cancels stale streams cleanly when Lens closes.
- **v2.5.3** — Lens screenshot selection drag no longer breaks when the cursor enters the chat bar area. History panel open no longer highlights or accidentally captures windows behind it. Restoring a conversation from history no longer shows the previous capture frame at stale coordinates.
- **v2.5.2** — Left-click the tray icon to open Lens directly (chat mode); right-click keeps the menu. Added a source/render toggle button next to Copy in the Lens answer area so you can switch between rendered Markdown and raw text. Lens chat mode now lets you send without typing anything — just hit Enter after capture and the model analyzes the screenshot. Apple Intelligence sidecar is now lazy-spawned on first use instead of at app startup, eliminating the startup delay when the binary is missing. Settings page no longer queries sidecar availability on open; it simply shows the Apple preset on macOS.
- **v2.5.1** — Lens screenshots can now be annotated with red arrows before going to the vision model. Toggle on the input bar, drag in the captured area, Cmd+Z to undo — the arrows are burned into the PNG so the model literally sees what you're pointing at. Polished Lens startup logging noise and taught provider failover to recognize the SSE-stream HTTP error format. Hardened the send flow so a stray Esc during composition no longer cancels the request. Trimmed leftover legacy template assets, slightly shrinking the install bundle.
...
- 📸 **截图翻译** —— 截任意窗口或选区，也可以直接翻译当前选中的文本，不必截图
...
截任意窗口或选区，译文浮窗飞到你选的位置，原文紧跟在下作参考。只想翻译一段已选文字时，按选中文本热键即可直接弹出同款翻译结果卡。
...
- **v2.5.5** —— 截图翻译在 macOS 和 Windows 都改为真正的系统 OCR：macOS 走 Apple Vision，Windows 走 `Windows.Media.Ocr`，识别后再把纯文本交给所选模型翻译。macOS OCR 拆成独立轻量 helper，不再依赖是否开启 Apple Intelligence。正式构建也会在必需 OCR helper 构建失败时直接中止。
- **v2.5.4** —— 截图翻译新增选中文本入口：选中文字后按 `⌘⇧T` / `Ctrl+Shift+T`，不用截图就能直接弹出翻译结果卡。设置页为这个入口新增独立快捷键录制项，并继续检测所有快捷键是否重复。选中文本结果卡支持拖动，复用截图翻译的流式、直译、显示原文和思考模式配置，关闭 Lens 时会取消旧流，避免污染下一次结果。
- **v2.5.3** —— Lens 截图选区拖动经过底部对话栏时不再中断。历史面板展开时不再高亮或误截图背后窗口。从历史记录恢复对话不再显示坐标过期的截图框。
- **v2.5.2** —— 托盘图标左键单击直接弹出 Lens（chat 模式），右键保留菜单。Lens 答案区复制按钮旁新增源码/渲染切换，可在 Markdown 渲染和原始文本之间切换。Lens chat 模式支持不输入文字直接发送 —— 截图后按回车即可让 AI 分析图片。Apple Intelligence sidecar 改为首次调用时才拉起，不再在 app 启动时 spawn，避免二进制缺失时的启动延迟。设置页打开时不再查询 sidecar 可用性，macOS 上直接显示 Apple 预设。
- **v2.5.1** —— Lens 截图发送给 vision 模型前可以拖红色箭头标注。输入栏左侧 toggle 进入，在截图区域内拖出箭头，Cmd+Z 撤销 —— 箭头会烧进 PNG，让模型看到你"指"的位置。Lens 启动路径清理了一批调试日志噪音，多 Key failover 现在也能识别 SSE 流报错格式。加固送图流程，合成期间误按 Esc 不再关闭 Lens。删了几个老模板残留资源，安装包略小。
