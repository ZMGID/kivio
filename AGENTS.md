...
KeyLingo is a lightweight desktop translation and AI vision utility for macOS and Windows. Its core focus is a small package size and low runtime footprint, providing instant text translation, screenshot translation, and screenshot explanation through global shortcuts.
...
src/                          # Frontend React + TypeScript source
  ...
  ScreenshotResult.tsx        # Screenshot translation result window
  ScreenshotExplain.tsx       # Screenshot explain / Q&A window
  CaptureOverlay.tsx          # Windows region capture overlay
...
src-tauri/
  src/                        # Rust backend source
    ...
    screenshot.rs             # Screenshot capture (macOS: screencapture / Windows: clipboard polling)
    settings.rs               # Settings data structures, serialization, migration, keyring storage
...
- `''` or `'translator'`: Main translator window (360x120, floating input)
- `'settings'`: Settings page (640x520)
- `'screenshot'`: Screenshot translation result window (500x400, standalone)
- `'explain'`: Screenshot explain window (700x800, standalone)
- `'capture'`: Windows region capture overlay (fullscreen transparent window)
...
- `explain_images: Mutex<HashMap<String, PathBuf>>` — Temporary image map for screenshot explain
- `pending_capture_mode: Mutex<Option<String>>` — Pending screenshot mode
- `screenshot_translation_busy / screenshot_explain_busy: AtomicBool` — Concurrency guards for screenshot operations

### Settings Persistence & Security

- Settings body is stored in Tauri Store as `settings.json`
- **API Keys are stored in the OS keyring**; the settings file keeps empty `api_key` strings
- On load, `sanitize_settings` cleans data and migrates legacy single-provider configs to the multi-provider system
- `settings.rs` contains full defaults, normalization logic (hotkeys, prompts), and keyring read/write helpers
...
- **Screenshot Explain**: `screenshot_explain.provider_id` + `model`

Each `ModelProvider` has `id`, `name`, `base_url`, `api_key`, `available_models`, and `enabled_models`.

### Platform-Specific Handling

- **macOS**:
  - Screenshots use the `screencapture -i` command for interactive region capture
...
1. Hotkey triggered -> hide main window
...
5. Otherwise: OCR result -> text translation -> emit `screenshot-result` event to the `ScreenshotResult` window

### Screenshot Explain Flow

1. Hotkey triggered -> capture image (same as above)
...
- Manual smoke-test checklist after changes:
  ...
  2. Global hotkeys (translator, screenshot translation, screenshot explain)
  3. Translation flow (input -> debounce -> result -> Enter to commit/paste)
  4. Screenshot translation / explain windows open correctly
...
- API Keys are stored in the OS keyring (`keyring` crate), not in `settings.json`
