...
- [ ] 添加评测 runner，输出 Recall@5/10/20、MRR、nDCG@10 和负样本误召回率。
...
- [ ] 处理 trigram <3 字符边界：过短 term 跳过或降级向量通道，不得拼进 MATCH。
...
- [ ] 若评测显示 term-OR 仍不足，评估 bigram 预处理（索引 2×，mangled 文本仅索引用）；否则记录为后续任务。
...
- [ ] 实现同文档相邻/高重叠 Chunk 去重或合并（在送 rerank 之前执行）。
