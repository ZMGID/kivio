...
| Spec | 内容 | 来源 |
|------|------|------|
| [压缩契约](./compaction-contracts.md) | `chat-compaction` 事件配对、boundary 双锚点（切分点 vs 时间线）、`_ui_message_id` runtime→UI 映射；链式摘要合并（`PERSISTED_SUMMARY_PREFIX` 共用）、估算口径（`estimate_value_tokens` 图片记0 / 展开工具转录）、多答排除臂过滤、取消≠失败 | 07-02-fix-compaction-stuck-and-boundary-mapping · 07-06-compaction-correctness-fixes |
