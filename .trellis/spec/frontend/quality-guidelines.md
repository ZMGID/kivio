...
- Windows frameless Chat uses a transparent shell gutter so CSS shadows are not clipped by the native window rectangle. Keep the gutter value synchronized across `src/index.css` (`.chat-window-host--win`), `src/chat/persistence.ts` (`CHAT_WINDOW_EDGE_GUTTER`), and `src-tauri/src/windows.rs` (`CHAT_WINDOW_EDGE_GUTTER`). Window minimum sizes and default sizes must add the gutter on Windows so the visible Chat content area still respects the base 400/660 x 400 minimum and 1280 x 800 default contracts.
